...
class FileExplorer(QDockWidget):
    ...
    file_execute_requested = Signal(str)
    ...
    CODE_EXTENSIONS = [
            ...
            "*.txt", "*.md", "*.json", "*.xml", 
            "*.bat", "*.sh", "*.vbs",
            "*.gitignore", "*.cmake", "CMakeLists.txt", "Makefile",
            ".inl", "*.natvis", "*.clang-format"
        ]
    
    EXECUTABLE_EXTENSIONS = {
            '.bat': 'cmd.exe /c "{file}"',
            '.vbs': 'wscript.exe "{file}"',
            '.sh': 'bash "{file}"',
            '.py': 'python "{file}"',
            '.exe': '"{file}"'
        }
    ...
    def setup_ui(self):
        ...
        self.model.setNameFilters(self.CODE_EXTENSIONS)
    ...
    def show_context_menu(self, position):
        ...
        if index.isValid():
            ...
            if not self.model.isDir(source_index) and file_ext in self.EXECUTABLE_EXTENSIONS:
                run_action = QAction(f"Executar {file_ext}", self)
                run_action.triggered.connect(lambda: self.file_execute_requested.emit(path))
    ...
    def execute_file(self, file_path, extension):
        try:
            import subprocess
            import platform
            
            commands = {
                '.bat': ['cmd.exe', '/c', file_path],
                '.vbs': ['wscript.exe', file_path],
                '.sh': ['bash', file_path],
                '.py': ['python', file_path],
                '.exe': [file_path]
            }
            ...
            if platform.system() == "Windows":
                if extension == '.sh':
                    QMessageBox.warning(self, "Aviso", "Scripts shell (.sh) não são nativos no Windows")
                    return
                
                subprocess.Popen(commands.get(extension, [file_path]), 
                               creationflags=subprocess.CREATE_NEW_CONSOLE)
            else:
                if extension in ('.bat', '.vbs'):
                    QMessageBox.warning(self, "Aviso", "Scripts .bat/.vbs são específicos do Windows")
                    return
